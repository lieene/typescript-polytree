...
namespace Typing
{
  ...
  export interface INodeEditing<TNode extends INodeEditing<TNode, TTree>, TTree extends ITreeEditing<TNode, TTree>>
  {
    ...
    push<N extends object>(ext: (node: TNode) => N): L.Extend<TNode, L.MergO<N>>;
    push<N extends object[]>(morph: 'morph', ...ext: N[]): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergTupleType<N>>>;
    push<N extends object>(morph: 'morph', ext: (node: TNode) => N): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>>;
  ...
  {
    poly<N extends object>(): L.Extend<TNode, L.MergO<N>>;
    morph<N extends object>(): SimpleMorphNode<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>, ExtOverTreeT<TTree>>;

    // poly<N extends object>(): TNode & N;
    // morph<N extends object>(): SimpleMorphNodeN<N>;
  }
  ...
  {
    poly<T extends object>(): L.Extend<TTree, L.MergO<T>>;
    morph<T extends object>(): SimpleMorphTreeT<L.Extend<ExtOverTreeT<TTree>, L.MergO<T>>>;
    polymorph<N extends object, T extends object>(): SimpleMorphTree<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>, L.Extend<ExtOverTreeT<TTree>, L.MergO<T>>>;

    // poly<T extends object>(): TTree & T;
  ...
  {
    poly<N extends object>(): L.Extend<TNode, L.MergO<N>>;
    morph<N extends object>(): MorphNodeN<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>>;

    // poly<N extends object>(): TNode & N;
    //morph<N extends object>(): MorphNodeN<N>;
  }
  ...
  {
    poly<T extends object>(): L.Extend<TTree, L.MergO<T>>;
    morph<T extends object>(): MorphTreeT<L.Extend<ExtOverTreeT<TTree>, L.MergO<T>>>;
    // polymorph<N extends object, T extends object>():
    //   MorphTree<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>, L.Extend<ExtOverTreeT<TTree>, L.MergO<T>>>;
  ...
  export interface IEditNode<TNode extends IEditNode<TNode, TTree>, TTree extends IEditTree<TNode, TTree>> extends INodeEditing<TNode, TTree>, INodeAccess<TNode, TTree>, INode2Tree<TTree>, INodeIdx
  {
    ...
    poly<N extends object>(ext: (node: TNode) => N): L.Extend<TNode, L.MergO<N>>;

    morph<N extends object[]>(...ext: N): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergTupleType<N>>>;
    morph<N extends object>(ext: (node: TNode) => N): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>>;

    polySub<N extends object[]>(...ext: N): L.Extend<TNode, L.MergTupleType<N>>;
    polySub<N extends object>(ext: (node: TNode) => N): L.Extend<TNode, L.MergO<N>>;

    morphSub<N extends object[]>(...ext: N): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergTupleType<N>>>;
    morphSub<N extends object>(ext: (node: TNode) => N): MorphNodeNX<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>>;


    // poly<N extends object[]>(...ext: N): TNode & L.MergTupleType<N>;
    // poly<N extends object>(ext: (node: TNode) => N): TNode & N;

    // morph<N extends object[]>(...ext: N): MorphNodeNX<L.MergTupleType<N>>;
    // morph<N extends object>(ext: (node: TNode) => N): MorphNodeNX<N>;

    // polySub<N extends object[]>(...ext: N): TNode & L.MergTupleType<N>;
    // polySub<N extends object>(ext: (node: TNode) => N): TNode & N;

    // morphSub<N extends object[]>(...ext: N): MorphNodeNX<L.MergTupleType<N>>;
  ...
  export interface IEditTree<TNode extends IEditNode<TNode, TTree>, TTree extends IEditTree<TNode, TTree>> extends ITreeEditing<TNode, TTree>, ITreeAccess<TNode, TTree>, ITree2Node<TNode>
  {
    ...
    morph<T extends object[]>(...ext: T): MorphTreeTX<L.Extend<ExtOverTreeT<TTree>, L.MergTupleType<T>>>;

    // polymorph<N extends object>(nodeExt: (node: TNode) => N): MorphTreeNX<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>>;
    // polymorph<N extends object[]>(...ext: N): MorphTreeNX<L.Extend<ExtOverNodeN<TNode>, L.MergTupleType<N>>>;
    // polymorph<N extends object, T extends object[]>(nodeExt?: (node: TNode) => N, ...ext: T):
    //   MorphTreeX<L.Extend<ExtOverNodeN<TNode>, L.MergO<N>>, L.Extend<ExtOverTreeT<TTree>, L.MergTupleType<T>>>;

    // poly<T extends object[]>(...ext: T): TTree & L.MergTupleType<T>;
    // morph<T extends object[]>(...ext: T): MorphTreeTX<L.MergTupleType<T>>;

    polymorph<N extends object>(nodeExt: (node: TNode) => N): MorphTreeNX<N>;
    polymorph<N extends object[]>(...ext: N): MorphTreeNX<L.MergTupleType<N>>;
    polymorph<N extends object, T extends object[]>(nodeExt?: (node: TNode) => N, ...ext: T): MorphTreeX<N, L.MergTupleType<T>>;
  ...
  export type SimpleMorphNode<N extends object, T extends object> = SimplePolyNode<N, T> & N;
  export type SimpleMorphNodeN<N extends object> = SimplePolyNode<N, object> & N;
  export type SimpleMorphNodeT<T extends object> = SimplePolyNode<object, T>;

  export type SimpleMorphTree<N extends object, T extends object> = SimplePolyTree<N, T> & T;
  export type SimpleMorphTreeT<T extends object> = SimplePolyTree<object, T> & T;
  ...
  export type ExtOverNodeN<TNode extends object> = L.ExcludeO<TNode, CNode>;
  export type ExtOverNodeT<TTree extends object> = L.ExcludeO<NodeType<TTree>, CNode>;
  export type ExtOverTreeN<TNode extends object> = L.ExcludeO<TreeType<TNode>, CTree>;
  export type ExtOverTreeT<TTree extends object> = L.ExcludeO<TTree, CTree>;
  ...
  export type Simplify<O extends Node | NodeX | Tree | TreeX> = O extends Node | NodeX
    ? SimpleMorphNode<ExtOverNodeN<O>, ExtOverTreeN<O>>
    : SimpleMorphTree<ExtOverNodeT<O>, ExtOverTreeT<O>>;

  export type Nomalize<O extends SimpleNode | NodeX | SimpleTree | TreeX> = O extends SimpleNode | NodeX
    ? MorphNode<ExtOverNodeN<O>, ExtOverTreeN<O>>
    : MorphTree<ExtOverNodeT<O>, ExtOverTreeT<O>>;

  export type Edit<O extends SimpleNode | Node | SimpleTree | Tree> = O extends SimpleNode | Node
    ? MorphNodeX<ExtOverNodeN<O>, ExtOverTreeN<O>>
    : MorphTreeX<ExtOverNodeT<O>, ExtOverTreeT<O>>;

  export function Simplify<N extends Node>(node: N): Simplify<N>;
  export function Simplify<N extends NodeX>(node: N): Simplify<N>;
  export function Simplify<T extends Tree>(tree: T): Simplify<T>;
  export function Simplify<T extends TreeX>(tree: T): Simplify<T>;
  export function Simplify<O extends Tree | TreeX | Node | NodeX>(obj: O): Simplify<O>
  {
    return obj as any;
  }

  export function Nomalize<N extends SimpleNode>(node: N): Nomalize<N>;
  export function Nomalize<N extends NodeX>(node: N): Nomalize<N>;
  export function Nomalize<T extends SimpleTree>(tree: T): Nomalize<T>;
  export function Nomalize<T extends TreeX>(tree: T): Nomalize<T>;
  export function Nomalize<O extends SimpleTree | TreeX | SimpleTree | NodeX>(obj: O): Nomalize<O>
  {
    return obj as any;
  }

  export function Edit<N extends SimpleNode>(node: N): Edit<N>;
  export function Edit<N extends Node>(node: N): Edit<N>;
  export function Edit<T extends SimpleTree>(tree: T): Edit<T>;
  export function Edit<T extends Tree>(tree: T): Edit<T>;
  export function Edit<O extends Tree | SimpleTree | Node | SimpleNode>(obj: O): any//Edit<O>
  {
    return obj as any;
  }
...
{
  let t: core.CTree = Tree(NamedTree.named(rootName)) as any;
  if (treeName !== undefined) { t.morph(NamedTree.named(treeName)); }
  return t as any;
}
